...
class PopPlot(object):
    ...
    cavity: boolean
        Indication for whether data was loaded from propagation of the full
        mode or the reduced model (where the cavity has been integrated out)
    dpi: int
    ...
    def load(self, runfolder, hilbert_space=True, cavity=True):
        ...
        If cavity is True, load from propagation of the full model, otherwise,
        from propagation of the reduced model (where the cavity has been
        integrated out)
        ...
        if hilbert_space:
            if cavity:
                ...
                files = [
                'psi00_q1.dat', 'psi00_q2.dat', 'psi00_phases.dat',
                'psi01_q1.dat', 'psi01_q2.dat', 'psi01_phases.dat',
                'psi10_q1.dat', 'psi10_q2.dat', 'psi10_phases.dat',
                'psi11_q1.dat', 'psi11_q2.dat', 'psi11_phases.dat'
                ]
        else:
            if cavity:
                files = [
                ...
                files = [
                'rho00_q1.dat', 'rho00_q2.dat', 'rho00_popdyn.dat',
                'rho01_q1.dat', 'rho01_q2.dat', 'rho01_popdyn.dat',
                'rho10_q1.dat', 'rho10_q2.dat', 'rho10_popdyn.dat',
                'rho11_q1.dat', 'rho11_q2.dat', 'rho11_popdyn.dat'
                ]
        self._peak_qubit_excitation = 0.0
        self._peak_cavity_excitation = 0.0
        if cavity:
    ...
    def render(self, basis_state, ax_pop, ax_q1, ax_q2, ax_cavity,
        ...
        ax_cavity: matplotlib.axes.Axes instance, None
            axes on which to render qubit cavity dynamics. May be None to
            indicate rendering of reduced model.
        pops: list
                ...
                if panel in ['q1', 'q2']:
                    ...
                    if panel == 'q2' and i_state == 0:
                        if self.cavity:
                            ...
                            'qubit and cavity excitation')
                        else:
                            current_ax.set_ylabel(
                            'qubit excitation')
                            current_ax.yaxis.label.set_position((0,0))
